...
export function scheduleResponse(string) {
  let parser = new DOMParser();
  let doc = parser.parseFromString(string, 'text/xml');
  let result = traverse.multistatus(child(doc, 'schedule-response'));
  debug(`input:\n${string}\noutput:\n${JSON.stringify(result)}\n`);
  return result;
}
...
let traverse = {
  ...
  response: node => complex(node, { propstat: true, href: false, recipient: false }),
  ...
  currentUserPrincipal: node => hrefchildren(node, false),
  scheduleInboxUrl: node => hrefchildren(node, false),
  scheduleOutboxUrl: node => hrefchildren(node, false),
  calendarHomeSet: node => hrefchildren(node, false),
  calendarUserAddressSet: node => hrefchildren(node, true),
  recipient: node => hrefchildren(node, false)
