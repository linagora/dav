import { multistatus, scheduleResponse } from './parser';
 ...
 */
export function calendarMultiget(options) {
  return collectionQuery(
    template.calendarMultiget({
      props: options.props || [],
      paths: options.paths || []
    }),
    {}
 ...
 */
export function freebusy(options) {
  function transformRequest(xhr) {
  ...
  function transformResponse(xhr) {
    return scheduleResponse(xhr.responseText).response;
  }

  let stamp = (new Date()).toISOString().replace(/([:-]|\.\d{3})/g, "");
  let requestData = [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//dav.js//freebusy",
    "METHOD:REQUEST",
    "BEGIN:VFREEBUSY",
    "DTSTAMP:" + stamp,
    "UID:" + stamp,
    ...
    ...options.attendees.map(att => "ATTENDEE;PARTSTAT=NEEDS-ACTION:" + att),
    "END:VFREEBUSY",
    "END:VCALENDAR"
  ].join("\n");
